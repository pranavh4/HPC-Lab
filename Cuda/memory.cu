...
// (for clarity, hardcoding 32 threads/elements and omitting out-of-bounds checks)
...
int main(int argc, char **argv)
{
     ...
     */
    use_local_memory_GPU<<<1, 32>>>(2.0f);

    /*
     * Next, call a kernel that shows using global memory
     */
    float h_arr[32];   // convention: h_ variables live on host
    float *d_arr;       // convention: d_ variables live on device (GPU global mem)

    // allocate global memory on the device, place result in "d_arr"
    cudaMalloc((void **) &d_arr, sizeof(float) * 32);
    // now copy data from host memory "h_arr" to device memory "d_arr"
    cudaMemcpy((void *)d_arr, (void *)h_arr, sizeof(float) * 32, cudaMemcpyHostToDevice);
    // launch the kernel (1 block of 32 threads)
    use_global_memory_GPU<<<1, 32>>>(d_arr);  // modifies the contents of array at d_arr
    // copy the modified array back to the host, overwriting contents of h_arr
    cudaMemcpy((void *)h_arr, (void *)d_arr, sizeof(float) * 32, cudaMemcpyDeviceToHost);
    // ... do other stuff ...
    ...
    cudaMemcpy((void *)h_arr, (void *)d_arr, sizeof(float) * 32, cudaMemcpyHostToDevice);
    // ... do other stuff ...
